# Contributing to Harvester MCP Server

Thank you for your interest in contributing to the Harvester MCP Server! This guide will help you get started with development and understand our contribution process.
...
```bash
git clone https://github.com/<your-username>/harvester-mcp-server.git
...
│   ├── virtualization/  # Harvester / SUSE Virtualization tools
...
To add a new tool to an existing toolset (e.g., adding a new tool to the `virtualization` toolset):

1. **Create a new handler file** in the toolset directory (e.g., `pkg/toolsets/virtualization/your_tool.go`):
...
3. **Create a test file** (e.g., `pkg/toolsets/virtualization/your_tool_test.go`) following the existing test patterns:
...
```bash
go test -v ./pkg/toolsets/virtualization
```
